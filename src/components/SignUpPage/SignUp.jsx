...
function SignUp() {
  ...
  const handleRegistration = async (e) => {
    ...
    try {
      const response = await fetch(`${localStorage.getItem('url')}/User/Register`, {
        method: "POST",
        headers: {
        ...
        <div class="max-w-md w-full p-6">
          ...
          <form onSubmit={handleRegistration} class="space-y-4">
            <div>
              ...
              <select id="userType" name="userType" value={role} onChange={handleRoleChange} class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300">
            ...
            <div>
              ...
              <input type="text" id="username" name="username" value={name} onChange={handleNameChange} class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300" />
            </div>
            <div>
            ...
            <div>
              <button type="submit" class="w-full bg-orange-500 text-white p-2 rounded-md hover:bg-gray-800 focus:bg-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 transition-colors duration-300">Sign Up</button>
            </div>
          </form>
